window.initGame = (React, assetsUrl) => {
  ...
  const Gobang = ({ assetsUrl }) => {
    ...
    const [firstPlayer, setFirstPlayer] = useState(1); // Player who goes first (Black or White)
    ...
    const handleReset = () => {
      ...
      setHistory([newBoard]);
      setCurrentIndex(0);
      setCurrentPlayer(firstPlayer); // Start with the selected first player
      setWinner(0);
      setTimer(60);
      setElapsedTime(0);
      setMoveRecords([]);
    ...
    const handleFirstPlayerChange = (player) => {
      ...
      // If the game is not in progress, reset the board and start with the new first player
      if (winner === 0 && currentIndex === 0) {
        handleReset();
      }
    };
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'select',
          { id: 'firstPlayer', onChange: (e) => handleFirstPlayerChange(parseInt(e.target.value)) },
          React.createElement(
            'option',
            { value: 1 },
            'Player 1 (Black)'
